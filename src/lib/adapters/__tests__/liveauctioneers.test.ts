...
describe('mapSearchItem', () => {
  ...
  it('maps status correctly', () => {
    expect(mapSearchItem({ ...mockSearchItem, catalogStatus: 'live' }, false).status).toBe('live');
    expect(mapSearchItem({ ...mockSearchItem, catalogStatus: 'online' }, false).status).toBe('online');
    expect(mapSearchItem({ ...mockSearchItem, catalogStatus: 'upcoming' }, false).status).toBe('upcoming');
    expect(mapSearchItem({ ...mockSearchItem, isSold: true }, false).status).toBe('sold');
    expect(mapSearchItem({ ...mockSearchItem, isPassed: true }, false).status).toBe('passed');
  });
});
...
describe('buildUnifiedItem', () => {
  it('combines data from all three sources', () => {
    const item = buildUnifiedItem('12345', mockItemDetail, mockItemFacets, mockContentItems);
  ...
  it('requires both estimate values for estimateRange', () => {
    ...
    const itemBoth = buildUnifiedItem('3', { estimateLow: 100, estimateHigh: 500 }, {}, {});
...
describe('LiveAuctioneersAdapter', () => {
  ...
  describe('search', () => {
    it('returns mapped search results', async () => {
      const mockFetch = createMockFetch([
        { ok: true, data: { error: false, payload: { items: [mockSearchItem] } } },
      ]);
    ...
    it('uses active auction status filters', async () => {
      const mockFetch = createMockFetch([{ ok: true, data: { error: false, payload: { items: [] } } }]);
      const adapter = new LiveAuctioneersAdapter({ fetchFn: mockFetch });
  ...
  describe('getPriceHistory', () => {
    ...
    it('includes sold price data', async () => {
      const mockFetch = createMockFetch([
        { ok: true, data: { error: false, payload: { items: [mockSoldItem] } } },
      ]);
      const adapter = new LiveAuctioneersAdapter({ fetchFn: mockFetch });
